...
const History = ({ item, navigation }) => {
  return (
    <BoxShadow style={styles.refundHistory}>
      <View style={styles.icon}>{checkIcon(parseInt(item?.status))}</View>
      <TouchableOpacity style={styles.wrapper}>
        <Text style={styles.date}>
          {new Date(item?.created_at).toISOString().substr(0, 10)}, request No.{" "}
        ...
        <Text style={styles.status}>
          {checkStatus(
            ...
            parseInt(item?.booking?.charges)
          )}
        </Text>
        <Text style={styles.date}>BOOKING ID. {item?.booking?.booking_id}</Text>
      </TouchableOpacity>
    </BoxShadow>
  );
};
  ...
  const fetchRefundRequest = async () => {
    ...
    try {
      ...
      if (response?.status) {
        ...
        console.log(response, "refund request failed");
      }
    } catch (error) {
      ...
      {loading && refund.length == 0 ? (
        ...
        <View style={{ flex: 1, marginHorizontal: -15 }}>
          <FlatList
            ...
            data={refund}
            renderItem={({ item }) => (
              <History item={item} navigation={navigation} />
            )}
