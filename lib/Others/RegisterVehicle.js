...
const RegisterVehicle = ({ navigation }) => {
  ...
  const checkForVehicles = async () => {
    dispatch(loadingState(true));

    try {
      const response = await getDataApi("getcurrentUserVehicles");
      if (response?.status) {
  ...
  return (
    <View style={styles.container}>
      <GoogleMap showsMyLocationButton={false} />
      ...
      <BottomSheet
        ...
        defaultPointIndex={1}
      >
        <View style={{ padding: 15, backgroundColor: "#FEFEFF" }}>
          <View style={{ marginBottom: 15 }}>
            <Title
              style={{ marginBottom: 0, marginTop: 0 }}
              text={userData?.name ? userData?.name : "Complete your Profile"}
            />
            <Subtitle
              text={
                userData?.phone != "null"
