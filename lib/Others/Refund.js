...
import { decodeAndFetchFirstValue, formatDateTime, postDataApi } from "../../src/helpers";
import { alertBox } from "../../src/helpers";
...
const RefundRequest = ({ navigation, route }) => {
  ...
  const raiseRefundRequest = async () => {
    ...
    let formatedData = {
      booking_id: data?.id,
      payment_id: decodeAndFetchFirstValue(data?.payment_ids),
      status: 0,
      reason: reason,
      notification_title:"Refund Request Raised",
      notification_message:`Parking ${data?.parking?.address} `,
      notification_type:"refund"
    };
    

    // // console.log(formatDateTime(),"I have this data as ")
    ...
    try {
      ...
      if (response?.status) {
        ...
        alertBox("Refund Error", response?.message);
