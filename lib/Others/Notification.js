...
import { hasValue } from "../../src/helpers";
...
const getData = (type, status, time, data) => {
  ...
  if (type == "booking") {
  ...
  } else if (type == "bookingExtend") {
    if (status == "success") {
      tempData.icon = <SuccessIcon />;
      tempData.nText = `Parking spot has been extended successfully`;
      return tempData;
    }
    if (status == "failed") {
      tempData.icon = <CancelIcon />;
      tempData.nText = `Parking spot extention failed..`;
    ...
    if (status == "success") {
      tempData.icon = (
        <Image
          source={require("../../assets/images/parking_purchased.png")}
      ...
      {data.length == 0 && (
        ...
        >
          ...
          <Text style={{ marginVertical: 10, textAlign: "center" }}>
            {loading ? "Looking for Current Bookings" : "No Notifications"}
          </Text>
        </View>
      )}
