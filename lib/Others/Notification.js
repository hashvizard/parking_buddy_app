...
const getData = (type, status, time, data) => {
  ...
  if (type == "booking") {
    if (status == "success") {
      ...
      tempData.nText = "Your parking spot has been successfully booked";

      return tempData;
    }
    if (status == "failed") {
      tempData.icon = <CancelIcon />;
      tempData.nText = "Booking Unsuccessful";
    ...
    if (status == "success") {
      tempData.icon = <SuccessIcon />;
      tempData.nText = "Successfully Bought the pass";

      return tempData;
    }
    if (status == "failed") {
      tempData.icon = <CancelIcon />;
      tempData.nText = "Failed to buy the pass";
