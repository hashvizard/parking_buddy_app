...
const BookingDetails = ({ navigation, parkingData }) => {
  console.log(parkingData?.parking_meta, "sddsdsd");
  ...
  useEffect(() => {
    let discount = cuponCode(
      ...
      getTimeFrame(
        ...
        parkingData?.parking_meta[0]?.rate
      )?.total_amount,
  ...
  const checkAvailability = async () => {
    let start_time = timestampToMysqlDatetime(booking?.selectedTime);
    let end_time = timestampToMysqlDatetimeWithOffset(booking?.selectedTime, parseInt(booking?.time_duration));
    ...
    try {
      ...
      if (response?.status) {
        ...
        if (tempdata.length > 0) {
          navigation.navigate("UserAvailablePasses", { passesData: tempdata });
        } else {
          makepayment();
        }
      } else {
        ...
        <TouchableOpacity onPress={() => navigation.navigate("Voucher")}>
          <BoxShadow style={styles.voucher}>
            {hasValue(booking?.cuponCode) ? (
              <TouchableOpacity
                onPress={() => dispatch(bookingData({ cuponCode: {} }))}
              >
                <Icon
                  name="trash-can"
                  size={30}
                  style={{ marginRight: 5, color: colors.orangeText }}
                />
              </TouchableOpacity>
            ) : (
              <VoucherIcon />
            ...
            <Text style={{ marginLeft: 10, marginRight: 25 }}>
              {hasValue(booking?.cuponCode)
                ...
                : "You have not selected any vouchers"}
            </Text>
          </BoxShadow>
        </TouchableOpacity>
          ...
          <BoxShadow style={styles.vehicle}>
            ...
            <View style={{ marginLeft: 20 }}>
              ...
              <Text style={{ fontWeight: "700", color: "#fecf3e" }}>
                ₹ {parkingData?.parking_meta[0]?.rate}/hour
              </Text>
            </View>
          </BoxShadow>
        ) : (
