...
import { filterArrayByEndTime, getDataApi } from "../../src/helpers";
...
const MyPasses = ({ navigation, route }) => {
  ...
  const fetchVoucers = async () => {
    ...
    try {
      ...
      if (response?.status) {
        let tempDataValidVouchers = filterArrayByEndTime(response?.data);
     
        setData(tempDataValidVouchers);
        try {
          let { screen } = route?.params;
          if (screen == "userPasses") {
            let tempdata = tempDataValidVouchers.filter(function (item) {
